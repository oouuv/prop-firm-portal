...
### 1.3 MinerU 解析常见问题与容错

MinerU 解析 PDF 经常出现以下问题，生成时需要处理：

| 问题 | 表现 | 处理方式 |
|------|------|----------|
| OCR 乱码 | 中文出现方块字、繁简混用 | 根据上下文推断修正，无法推断时保留原文并标注 |
| 图片引用断裂 | `![](images/xxx.jpg)` 但文件不存在 | 在 HTML 中标注"图片缺失"，不使用破损链接 |
| 表格错位 | 列对不齐、合并单元格丢失 | 手动重排表格结构，保持信息完整 |
| 公式丢失 | 数学符号变成问号或乱码 | 根据上下文还原公式，用 HTML 实体或 Unicode 表示 |
| 分页断裂 | 一句话被分到两页，中间断开 | 拼接为完整句子后再分析 |
| 图表编号错误 | "图 21.1" 被解析为其他格式 | 根据上下文推断正确编号 |

**验证步骤**：在内容分析前，先用 `grep` 检查章节范围内所有图片引用，确认每个图片文件都存在。缺失的图片要在 HTML 中用占位符标注。
...
### Step 4：设计教学模块
将拆解后的内容组织为 4-7 个教学模块，每个模块聚焦一个主题。
...
| 章节特征 | 推荐模块组合 | 说明 |
|----------|-------------|------|
| 概念多 + 规则多 | 核心定义 → 原理深入 → 交易规则 → 形态变种 → 总结自测 | 标准流程 |
| 图表多（10+张） | 核心定义 → 图表精讲(1) → 图表精讲(2) → 交易规则 → 总结自测 | 图表可拆成两个模块 |
| 理论强 + 规则少 | 核心定义 → 原理深入 → 案例分析 → 对比辨析 → 总结自测 | 侧重理解而非操作 |
| 有计算/测量内容 | 核心定义 → 原理深入 → 测量运动 → 交易规则 → 总结自测 | 计算器模块前置 |
| 短章节（<200行） | 核心定义 → 要点精讲 → 总结自测 | 精简到 3-4 个模块 |
| 有明确的形态分类 | 核心定义 → 形态A → 形态B → 形态C → 交易规则 → 总结自测 | 按形态拆分 |

#### 短章节处理（<200行）

当章节内容较少时：
1. **不强行拆分**：3-4 个模块即可，不需要凑到 6 个
2. **合并相关内容**：可以把"原理"和"规则"合并到一个模块
3. **减少卡片数量**：每个模块 3-5 张卡片即可
4. **保持完整性**：即使短，也要有"核心定义"和"总结自测"
5. **不减少交互**：仍然要有术语提示、lightbox、测验等交互元素

#### 长章节处理（>500行）

当章节内容非常丰富时：
1. **拆分模块**：可以增加到 7-8 个模块
2. **图表分批**：10+ 张图表可以拆成两个模块
3. **变种独立**：每种形态变种可以单独一个模块
4. **导航分组**：导航栏可以用分隔线或分组区分模块
...
每个章节的 HTML 包含完整的 CSS（~400行）和 JS（~200行），内容完全一致。为保证一致性并减少重复：

**直接复制**：每个新章节的 CSS 和 JS 代码块与已完成章节（如第21章）完全相同，直接复制即可。不要"重新写一遍"，否则会产生细微差异。

**修改原则**：
- 只允许修改导航栏的 `header-nav` 链接（模块名称和数量）
- 只允许修改 Hero 区域的标题和统计数字
- 不允许修改 CSS 变量、卡片样式、JavaScript 逻辑
- 如需新增 CSS 组件（如特殊的交互效果），必须在新章节文件末尾追加，不修改已有样式

**验证**：新章节的 CSS 变量和核心类名必须与第21章完全一致。
...
以下术语在各章节中反复出现，定义应保持一致。使用 `<span class="glossary">` 时，tooltip 内容参考此表：

| 术语 | 英文 | 标准定义 |
|------|------|----------|
| 尖峰 | Spike | 一组强劲的K线，通常是对突破的反应，形成急剧的价格运动 |
| 通道 | Channel | 倾斜的交易区间，价格在两条平行线之间波动 |
| 逃逸缺口 | Runaway Gap | 趋势中途出现的缺口，表示趋势加速，通常不会被快速回补 |
| 总在场内 | Always In | 市场处于明确趋势中，应该始终持有头寸（多头或空头） |
| 交易区间 | Trading Range | 价格在水平边界内波动，多空力量均衡 |
| 突破 | Breakout | 价格突破重要的高点或低点 |
| 回调 | Pullback | 趋势中的反向运动，是对趋势的测试 |
| 测量运动 | Measured Move | 基于前期价格运动幅度预测目标位 |
| 信号棒 | Signal Bar | 发出交易信号的特定K线形态 |
| 入场棒 | Entry Bar | 实际触发入场的K线 |
| 高潮 | Climax | 趋势末端的极端价格运动，通常伴随成交量放大 |
| 双顶/双底 | Double Top/Bottom | 两次测试同一价格水平后反转 |
| 楔形 | Wedge | 三条趋势线逐渐收敛的形态 |
| 头肩顶/底 | Head & Shoulders | 中间高点（头）两侧各一个较低高点（肩）的反转形态 |
| 均线 | Moving Average | 价格的平均值连线，常用20期EMA |

**使用规则**：
1. 首次出现时用 glossary tooltip 展示完整定义
2. 后续出现时可直接使用，不重复 tooltip
3. 不同章节对同一术语的解释必须一致
...
| 章节类型 | 题目数 | 难度分布 |
|----------|--------|----------|
| 标准章节 | 8-10 题 | 基础 3-4 题 + 中等 3-4 题 + 进阶 1-2 题 |
| 短章节 | 5-6 题 | 基础 2-3 题 + 中等 2-3 题 |
| 长章节 | 10-12 题 | 基础 3-4 题 + 中等 4-5 题 + 进阶 2-3 题 |

#### 题目类型（至少覆盖 3 种）

1. **概念题**：测试对核心定义的理解
   - 示例："以下关于通道的描述，哪个是正确的？"
2. **应用题**：测试在具体场景中运用规则的能力
   - 示例："在一个强上升通道中，价格触及通道下轨并形成看涨信号棒，你应该怎么做？"
3. **图表识别题**：展示图表片段，要求识别形态
   - 示例："图中棒5-棒8形成了什么形态？"
4. **判断题**：判断某个说法的对错
   - 示例："通道很少顺势突破。这个说法是否正确？"
5. **反直觉题**：测试是否理解 Al Brooks 的反直觉洞察
   - 示例："在市场看起来最弱的时候，最好的交易策略是什么？"

#### 答案解析要求

每道题的 `explanation` 必须包含：
1. **正确答案的解释**：为什么这个选项正确
2. **错误选项的分析**：为什么其他选项错误（至少分析 1-2 个干扰项）
3. **关联原文**：如果可能，引用原文中的相关句子

#### 题目质量红线

- ❌ 不要出"以下哪个是正确的"但所有选项都对的题
- ❌ 不要出过于简单的题（如"市场有两种状态：趋势和交易区间，对吗？"）
- ❌ 不要出过于刁钻的题（需要死记硬背具体页码或段落编号）
- ❌ 不要出多个正确答案的题（除非明确标注"多选"）
- ✅ 每道题都应该测试一个具体的交易概念或规则
- ✅ 题目之间不应有大量重复（如 3 道题都问通道的定义）
...
单 HTML 文件可能包含 10-20 张大图，需要考虑加载性能：

| 优化措施 | 说明 |
|----------|------|
| `loading="lazy"` | 所有图片必须添加，延迟加载视口外的图片 |
| `width` + `height` | 预设图片尺寸，避免布局偏移（CLS） |
| 图片压缩 | 如果图片 > 500KB，建议用工具压缩到 200-300KB |
| WebP 格式 | 如果原图是 JPG 且 > 300KB，可转换为 WebP 减小体积 |
| 缩略图 | 对于特别大的图（> 1MB），可在卡片中显示缩略图，lightbox 中显示原图 |

**HTML 中的图片标签模板**：
```html
<img src="..." alt="图 XX.N 描述" loading="lazy" width="800" height="450" style="border-radius: 6px; max-width: 100%; height: auto;">
```
...
### 审阅与迭代
- [ ] 在浏览器中完整浏览一遍，记录需要修改的地方
- [ ] 修改时只改需要改的卡片，不触动其他内容
- [ ] 修改后重新验证受影响的功能（交互、样式）
- [ ] 保留修改记录（如"2024-01: 修正了图21.3的逐棒分析"）
...
6. **统计原文覆盖率**：
   - 金句（⭐⭐⭐）：必须 100% 覆盖，不允许遗漏
   - 重要概念（⭐⭐）：必须覆盖，可用原文引用或解读形式
   - 辅助说明（⭐）：可概括处理，但整体信息量不丢失
   - 总体原则：章节的核心观点和交易规则不能有遗漏
...
当需要连续生成多个章节时，遵循以下工作流：

### 13.1 生成顺序建议

| 策略 | 适用场景 | 说明 |
|------|----------|------|
| 按书籍顺序 | 从头开始制作 | 保证跨章节引用的逻辑顺序 |
| 先易后难 | 快速建立信心 | 短章节/图表多的章节优先 |
| 按主题聚类 | 专题学习 | 如"所有通道相关章节"一起做 |

### 13.2 跨章节引用处理

- **前向引用**：如果当前章节提到后续章节的概念，在卡片中用文字标注"详见第XX章"，不创建链接（因为目标页面可能还不存在）
- **后向引用**：如果当前章节引用已完成的章节，创建导航链接 `<a href="../al-brooks-chXX/index.html">第XX章</a>`
- **术语一致性**：引用其他章节的术语时，使用本提示词 5.5 节的标准定义

### 13.3 术语一致性检查

每生成一个新章节后：
1. 检查本章使用的术语是否与已有章节一致
2. 如有新增术语，更新 5.5 节的术语表
3. 确保同一术语在不同章节的 tooltip 定义相同

### 13.4 导航栏更新

每个新章节生成后，需要更新导航栏的章节列表。建议维护一个全局导航配置：

```html
<!-- 所有章节的导航链接 -->
<div class="header-nav">
  <a href="../al-brooks-ch21/index.html">Ch.21 尖峰和通道</a>
  <a href="../al-brooks-chXX/index.html">Ch.XX 章节名</a>
  <!-- 新增章节在此追加 -->
</div>
```
...
- [ ] 跨章节引用链接有效
- [ ] 导航栏包含所有已完成章节
- [ ] 文件命名规范一致（`al-brooks-chXX/index.html`）
